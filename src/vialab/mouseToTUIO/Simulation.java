		...
		if (selectedCursor!=null){
			if (table.contains(pt)) {
				...
				if (table.contains(pt)) {
					...
					cursorMessage();
					if (me.isShiftDown() || SwingUtilities.isRightMouseButton(me)) stickyCursors.addElement(selectedCursor.sessionID);
				}
			}
	...
	public void mouse_pressed (MouseEvent me) {
		...
		while (cursorList.hasMoreElements()) {
			...
			if (point.distance(x,y)<7) {
				...
				if ((me.isShiftDown() || SwingUtilities.isRightMouseButton(me)) && selCur != cursor.sessionID) {
					...
					selectedCursor = null;
					return;
				} else if (me.isControlDown() || SwingUtilities.isMiddleMouseButton(me)) {
					if (jointCursors.contains(cursor.sessionID)) jointCursors.removeElement(cursor.sessionID);
		...
		if (me.isControlDown() || SwingUtilities.isMiddleMouseButton(me)) return;
